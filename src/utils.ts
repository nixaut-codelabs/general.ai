...
import type {
  ...
  GeneralAISpeedLabel,
  GeneralAISpeedMeasurement,
  GeneralAIStepPerformance,
...
export function aggregateUsage(
  ...
  return entries.reduce<GeneralAIUsageSummary>(
    (summary, entry) => {
...
export function summarizeUsageEntry(entry: unknown): GeneralAIUsageSummary {
  const empty: GeneralAIUsageSummary = {
    inputTokens: 0,
    outputTokens: 0,
    totalTokens: 0,
    cachedInputTokens: 0,
  ...
  if (!entry || typeof entry !== "object" || !("usage" in entry) || !(entry as any).usage) {
    return empty;
  }

  const usage = (entry as { usage: ResponseUsage | CompletionUsage }).usage;
...
function classifySpeed(value: number, mode: GeneralAISpeedMeasurement["mode"]): GeneralAISpeedLabel {
  if (mode === "stream_tps") {
    if (value < 10) {
      return "very_slow";
    }
...
function createHeuristicSpeedMeasurement(
  ...
  return {
    ...
    algorithm:
      "speed_index = (output_tokens + input_tokens * 0.15) / request_seconds. This is a heuristic, not real TPS.",
  };
}

function createStreamTpsMeasurement(
  outputTokens: number,
  ...
  return {
    ...
    algorithm:
      "stream_tps = output_tokens / output_stream_seconds, where output_stream_seconds starts at the first received text delta and ends at the last received text delta (or step completion if only one delta arrives).",
  };
}

function resolveOutputWindowMs(step: StepTimingWindow): number | undefined {
...
function createStepPerformance(
  ...
  return omitUndefined({
    ...
    firstTokenLatencyMs:
      firstTokenLatencyMs !== undefined ? roundMetric(firstTokenLatencyMs) : undefined,
...
export function buildPerformanceSummary(params: {
  ...
  const steps = params.steps.map((step, index) =>
    createStepPerformance(step, summarizeUsageEntry(params.endpointResults[index])),
  );
  const usage = aggregateUsage(params.endpointResults as Array<Response | ChatCompletion | undefined>);
  ...
  const timeToFirstTokenMs =
    firstStreamingStep?.firstTextDeltaAt !== undefined
      ? roundMetric(Math.max(firstStreamingStep.firstTextDeltaAt - params.runStartedAt, 0))
      : undefined;
  const totalOutputWindowMs = steps.reduce(
    (total, step) => total + (step.outputWindowMs ?? 0),
    0,
  );
  const speed =
    totalOutputWindowMs > 0
  ...
  return omitUndefined({
    ...
    speed,
    steps,
  }) as GeneralAIPerformanceSummary;
}
