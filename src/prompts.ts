...
import type {
  ...
  GeneralAIPromptPack,
  GeneralAIRenderedPromptSection,
  GeneralAIRenderedPrompts,
  PromptSectionKey,
} from "./types.js";

const PROMPT_FILES: Record<PromptSectionKey, string> = {
...
let bundledPromptCache:
  | Promise<Record<PromptSectionKey, string>>
  | undefined;
...
async function loadBundledPromptTemplates(): Promise<Record<PromptSectionKey, string>> {
  bundledPromptCache ??= (async () => {
    const entries = await Promise.all(
      PROMPT_SECTION_ORDER.map(async (key) => {
        const file = resolve(bundledPromptDir, PROMPT_FILES[key]);
        return [key, await readPromptFile(file)] as const;
      }),
    );

    return Object.fromEntries(entries) as Record<PromptSectionKey, string>;
  })();

  return await bundledPromptCache;
}

async function loadPromptPackTemplates(
  promptPack: GeneralAIPromptPack | undefined,
): Promise<Partial<Record<PromptSectionKey, string>>> {
  ...
  const entries = await Promise.all(
    PROMPT_SECTION_ORDER.map(async (key) => {
      ...
      return [key, await readPromptFile(filePath)] as const;
    }),
  );
...
export async function renderPromptSections(options: {
  ...
  const mergedTemplates: Partial<Record<PromptSectionKey, string>> = {
    ...bundled,
    ...fromPack,
  ...
  const sections: GeneralAIRenderedPromptSection[] = [];
